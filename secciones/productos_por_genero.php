<?PHP
$genero = $_GET['genero'] ?? FALSE;
$miObjetoDisco = new Disco();
$discos = $miObjetoDisco -> coleccion_por_genero($genero);
$miTituloSecciongenero = ucfirst(str_replace("_", " ", $genero ));
?>

<h3 class="text-center my-5"><?=$titulo; ?><?=$miTituloSecciongenero; ?></h3>
	...
	<div class="row mb-3">
		...
		<a href="index.php?sec=detalle&id=<?= $producto->id; ?>">
		 ...
		 <div class ="col-md">
			...
			<p>Artista:<?= $producto->artista_formateado() ?></p>
			<p>Descripcion:<?= $producto->descripcion; ?></p>
			<p><strong>Genero: <?= $producto->genero_formateado() ?></strong> </p>
		</div>
	</div>
    <?PHP } ?>
<?PHP }else{ ?>
<p class="lead">No se encontro el genero solicitado</p>
<?PHP } ?>
</div>
